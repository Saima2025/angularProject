...
export class CourseService {
  private storageKey = 'lms-courses';
  private ENROLLMENTS_KEY = 'enrollments';


  private sampleCourses: Course[] = [ {
  ...
  private sampleEnrollments: Enrollment[] = [
  {
  ...
  constructor() {
    if (!localStorage.getItem(this.storageKey) || localStorage.getItem(this.storageKey) == '[]') {
      localStorage.setItem(this.storageKey, JSON.stringify(this.sampleCourses));
    }
     const existing = localStorage.getItem(this.ENROLLMENTS_KEY);
    if (!existing) {
        localStorage.setItem(this.ENROLLMENTS_KEY, JSON.stringify(this.sampleEnrollments));
    }
  }
